...
class FakeCommands:
    def __init__(self):
        self.sent_msgs: list[tuple[dict, str]] = []
...
class FakeMeshCore:
    ...
    async def deliver_dm(self, text, pubkey_prefix="abcdef123456"):
        event = FakeEvent(
            EventType.CONTACT_MSG_RECV,
            {"type": "PRIV", "pubkey_prefix": pubkey_prefix, "text": text},
        )
        await self.callbacks[EventType.CONTACT_MSG_RECV](event)

    async def deliver_chan(self, text, channel_idx=0):
        event = FakeEvent(
            EventType.CHANNEL_MSG_RECV,
            {"type": "CHAN", "channel_idx": channel_idx, "text": text},
        )
...
class TestLifecycle:
    async def test_start_subscribes_and_begins_fetching(self, runner, mc):
        assert EventType.CONTACT_MSG_RECV in mc.callbacks
        assert EventType.CHANNEL_MSG_RECV in mc.callbacks
        assert mc.fetching

    async def test_stop_unsubscribes_and_stops_fetching(self, runner, mc):
        await runner.stop()
        assert mc.callbacks == {}
...
class TestDirectMessages:
    async def test_dm_command_replies_to_sender(self, runner, mc):
        await mc.deliver_dm("!ping")
        assert mc.commands.sent_msgs == [(ALICE, "pong")]

    async def test_dm_sender_name_comes_from_contact(self, runner, mc):
        await mc.deliver_dm("!whoami")
        assert mc.commands.sent_msgs == [(ALICE, "alice")]

    async def test_dm_from_unknown_contact_refreshes_then_ignores(self, runner, mc):
    ...
    async def test_non_command_dm_sends_nothing(self, runner, mc):
        await mc.deliver_dm("hello bot")
        assert mc.commands.sent_msgs == []

    async def test_failed_send_does_not_raise(self, runner, mc):
...
class TestChannelMessages:
    async def test_channel_command_replies_on_same_channel(self, runner, mc):
        await mc.deliver_chan("alice: !ping", channel_idx=2)
        assert mc.commands.sent_chan_msgs == [(2, "pong")]

    async def test_sender_name_parsed_from_text_convention(self, runner, mc):
        await mc.deliver_chan("alice: !whoami")
        assert mc.commands.sent_chan_msgs == [(0, "alice")]

    async def test_text_without_name_prefix_still_dispatches(self, runner, mc):
        await mc.deliver_chan("!ping")
        assert mc.commands.sent_chan_msgs == [(0, "pong")]

    async def test_channel_messages_ignored_when_bot_disabled_for_channels(self, mc):
